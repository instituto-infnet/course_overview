...
class block_course_overview extends block_base {
    ...
    public function get_content() {
        ...
        //print_object($ctab->sortedcourses);

        $btab = new stdClass;
        $btab->tab = 'blocos';
        list($btab->sortedcourses, $btab->sitecourses, $btab->totalcourses)
            = block_course_overview_get_sorted_courses(false, $config->keepfavourites, array_keys($ftab->sortedcourses));
        $disciplinas = new stdClass;

        $index = 0;
        $temp;
        foreach ($btab->sortedcourses as $sortedcourses) {
            if ($temp != $sortedcourses->category) {
                $index = $sortedcourses->category;
                $disciplinas->$index->id = $sortedcourses->category;
                $disciplinas->$index->category = $sortedcourses->category;
                //$disciplinas->$index->fullname = 'CATEGORY_NAME';
            }
            $temp = $sortedcourses->category;
        }

        $btab->sortedcourses = $disciplinas;

        $btab->overviews = block_course_overview_get_overviews($btab->sortedcourses);
